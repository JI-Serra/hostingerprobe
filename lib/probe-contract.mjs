...
export function databaseConfigurationStatus(environment = process.env) {
  return {
    configured: DATABASE_ENVIRONMENT_KEYS.every((key) => typeof environment[key] === 'string' && environment[key].trim() !== ''),
    status: DATABASE_ENVIRONMENT_KEYS.every((key) => typeof environment[key] === 'string' && environment[key].trim() !== '') ? 'configured' : 'pending'
  };
}
